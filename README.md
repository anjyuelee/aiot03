...
- **行政區圖層**：全台時各縣市分色並標上中文名稱，進入縣市後改為其中鄉鎮分色、其他縣市變淡；以地圖著色（DSatur 回溯）讓相鄰區域不同色，縣市 3 色、鄉鎮 4 色，配色經色盲模擬檢查
...
flowchart LR
  subgraph Browser["瀏覽器（React + Vite SPA）"]
    ...
    Render["繪圖 lib<br/>IDW 熱圖 · 風粒子 · 鄉鎮 choropleth<br/>縣市／鄉鎮界線與分色 · Mercator 重投影 · 雲層處理"]
    Map["MapLibre GL"]
    UI <--> Store
    Store --> Query
    Query --> Render --> Map
